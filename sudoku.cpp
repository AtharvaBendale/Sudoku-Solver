...
//     private:
//     int board[9][9];
//     // Initializes a 9x9 array representing the elements of the sudoku board.
...
//         for(int k=0; k<9; k++){
//             if(board[i][k] != 0){ truth_value[board[i][k]-1] = 1; } // Checking numbers along the column
//             if(board[k][j] != 0){ truth_value[board[k][j]-1] = 1; } // Checking numbers along the row
//         }

//         int sq_i = (i/3), sq_j = (j/3);
//         sq_i *= 3; sq_j *= 3;
...
//     // Function to copy one sudoku boards contents into another.
//     void copier(int source[][9], int sink[][9]){
//         for(int i=0; i<9; i++){
//             for(int j=0; j<9; j++){
//                 sink[i][j] = source[i][j];
//             }
...
//             if(awol){   copier(buf_obj.board, board); /**this = buf_obj;*/   return 1;   }
